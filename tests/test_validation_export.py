...
def test_validation_scientifique_export(config):
    ...
    pop_totale = gdf[population_column].sum()
    batiments_occupes = len(gdf[gdf[population_column] > 0])
    ...
    gdf['densite_relative'] = gdf[population_column] / gdf.geometry.area
    ...
    densite_moyenne_globale = gdf[gdf[population_column] > 0]['densite_relative'].mean()
    ...
    moyenne_senior = gdf[gdf[population_column] > 0]['prob_senior'].mean()
